...
async function handler(req, res, agencyId) {
  if (agencyId !== undefined) {
    if (req.method === "GET") {
      log.info(agencyId + " getting unclaimed conversions files");
      ...
      if (response.status === 200) {
        ...
        log.error(
          `Exception caught in /api/conversions/unclaimed while calling ${
            response.url
      ...
      log.info(`/api/conversions/unclaimed called with unsupported method: ${req.method}`);
